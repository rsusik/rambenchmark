...
double getWorst(map<int, vector<double>> res, int th) {
	return *max_element(res[th].begin(), res[th].end());
}

pair<int, double> getWorst(map<int, vector<double>> res) {
...
double getBest(map<int, vector<double>> res, int th) {
	return *min_element(res[th].begin(), res[th].end());
}

pair<int, double> getBest(map<int, vector<double>> res) {
	...
	cout << nth << " concurrent threads are supported." << endl << endl;
{
	...
	auto max_overall = getBest(res);
	auto min_overall = getWorst(res);
	...
	for (int i = 1; i <= nth; i++) {
		cout << "" <<
			...
			((double)(BUFFER_SIZE/1000/1000)) / getBest(res, i) << " (MB/s)" << endl;
	}
	cout << endl;
}
